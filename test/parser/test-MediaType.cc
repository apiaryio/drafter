...
TEST_CASE("parse valid type/just+last+part+is+suffix") {
    ...
    state result;
    tao::pegtl::memory_input<> in("type/just+last+part+is+suffix","");
    REQUIRE(tao::pegtl::parse<match_grammar, action/*, errors */>(in, result));
...
TEST_CASE("missing subtype") { 
    ...
    REQUIRE_THROWS_WITH((tao::pegtl::parse<match_grammar, action/*, errors */>(in, result)), ":1:4(4): parse error matching parser::mediatype::slash");
}

TEST_CASE("invalid char in type") { 
    state result;
    tao::pegtl::memory_input<> in("ty?pe/subtype","");
    REQUIRE_THROWS_WITH((tao::pegtl::parse<match_grammar, action/*, errors */>(in, result)), ":1:2(2): parse error matching parser::mediatype::slash");
}

TEST_CASE("double slash") { 
    state result;
    tao::pegtl::memory_input<> in("type/sub/type","");
    REQUIRE_THROWS_WITH((tao::pegtl::parse<match_grammar, action/*, errors */>(in, result)), ":1:8(8): parse error matching parser::mediatype::bad_slash");
}
