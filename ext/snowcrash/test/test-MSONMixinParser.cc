...
TEST_CASE("Parse canonical mson mixin", "[mson][mixin]")
{
    ...
    SectionParserHelper<mson::Mixin, MSONMixinParser>::parseMSON(
        source, MSONMixinSectionType, mixin, ExportSourcemapOption, namedTypes);
...
TEST_CASE("Parse mson mixin with base type definition", "[mson][mixin]")
{
    ...
    ParseResult<mson::Mixin> mixin;
    SectionParserHelper<mson::Mixin, MSONMixinParser>::parse(
        source, MSONMixinSectionType, mixin, ExportSourcemapOption);
