...
namespace refract
{
    ...
    void JSONSchemaVisitor::anyOf(std::map<std::string, std::vector<IElement*>>& types, std::vector<std::string>& types_order) {
        ArrayElement *a = new ArrayElement;
        a->renderType(IElement::rCompact);

        for (std::vector<std::string>::const_iterator i = types_order.begin();
             i != types_order.end();
             ++i) {
    ...
    void JSONSchemaVisitor::enumElement(const ArrayElement& e, const ArrayElement::ValueType *val) {
        ...
        for (ArrayElement::ValueType::const_iterator it = val->begin();
            ...
            if (*it) {
                ...
                if (items.empty()) types_order.push_back((*it)->element());
                items.push_back(*it);
            }
        }

        if (types.size() > 1) {
            anyOf(types, types_order);
        } else {
    ...
    IElement* JSONSchemaVisitor::get() {
        // std::cout << "JSONSchemaVisitor::get() --- \n";
        // PrintVisitor ps(0, std::cout);
        // ps.visit(*pObj);
        // std::cout << "--- JSONSchemaVisitor::get()\n";
        return pObj;
    }
